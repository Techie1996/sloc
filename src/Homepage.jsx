...
import revertLogo from '../assets/Imgs/Group (1).svg'
...
useEffect(() => {
  const logoEl = logoRefs.current;
  const welcomeEl = welcomeTextRef.current;
  const containerEl = containerRefs.current;
  const scrollImageEl = scrollImageRef.current;

  if (!logoEl || !welcomeEl || !containerEl || !scrollImageEl) return;

  // Animate logo movement with scroll
  ...
  // Color change based on scroll direction & overlap
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      const isOverlapping =
        logoRect.bottom > welcomeRect.top &&
        logoRect.top < welcomeRect.bottom;

      const direction = self.direction;

      if (isOverlapping && !wasOverlapping) {
        if (direction === 1) {
          // Scrolling down → logo overlaps welcome → turn blue
          gsap.to(logoEl.querySelectorAll('.logo-main, .logo-secondary'), {
            fill: '#007BFF',
            duration: 0.3,
          });
        } else if (direction === -1) {
          // Scrolling up → logo reenters from bottom → revert to original
          gsap.to(logoEl.querySelectorAll('.logo-main, .logo-secondary'), {
            fill: '#343434',
            duration: 0.3,
          });
